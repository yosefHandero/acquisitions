...
const clients = {
  admin: aj.withRule(
    slidingWindow({ mode: 'LIVE', interval: '1m', max: 20, name: 'admin-rl' })
  ),
  user: aj.withRule(
    slidingWindow({ mode: 'LIVE', interval: '1m', max: 10, name: 'user-rl' })
  ),
  guest: aj.withRule(
    slidingWindow({ mode: 'LIVE', interval: '1m', max: 5, name: 'guest-rl' })
  ),
};
